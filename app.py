...
def is_blocked_domain(host: str) -> bool:
    ...
    return any(
        host_lower == blocked or host_lower.endswith("." + blocked)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if resp.status == 200:
                    ...
                    ISP = (
                        f"{data.get('country_code', '')}-{data.get('isp', '')}".replace(
                            " ", "_"
                        )
                    )
                    return
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if resp.status == 200:
                    data = await resp.json()
                    ISP = (
                        f"{data.get('countryCode', '')}-{data.get('org', '')}".replace(
                            " ", "_"
                        )
                    )
                    return
...
class ProxyHandler:
        ...
        try:
            ...
            if atyp == 1:  # IPv4
                ...
                if i + 16 > len(first_msg):
                    return False
                host = ":".join(
                    f"{(first_msg[j] << 8) + first_msg[j+1]:04x}"
    ...
    async def handle_trojan(self, websocket, first_msg: bytes) -> bool:
        try:
            ...
            if (
                received_hash_hex != expected_hash_hex1
                and received_hash_hex != expected_hash_hex2
            ):
                return False
...
async def main():
    ...
    if not is_port_available(actual_port):
        logger.warning(
            f"Port {actual_port} is already in use, finding available port..."
        )
